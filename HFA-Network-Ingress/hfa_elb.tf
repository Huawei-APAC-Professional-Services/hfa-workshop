...
resource "huaweicloud_elb_loadbalancer" "hfa_transit_prod_ingress" {
  ...
  vpc_id         = data.terraform_remote_state.hfa_network.outputs.hfa_transit_ingress_prod_id
  ipv4_subnet_id = data.terraform_remote_state.hfa_network.outputs.hfa_transit_ingress_prod_public_id

  l4_flavor_id = data.huaweicloud_elb_flavors.hfa_transit_ingress_prod_elb_flavor.ids[0]

  availability_zone = var.hfa_ingress_elb_zones
